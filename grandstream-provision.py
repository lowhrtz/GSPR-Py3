...
def get_config(path_info):
    ...
    try:
        ast_db = mysql.connect(host=settings[6], user=settings[7], passwd=settings[8], db=settings[9])
        ...
        return 'Problem with MySQL/MariaDB database.' + '\nError: '  + str(e) #+ '\nSettings: ' + '|'.join(settings[6:10])
    string_format = {
        'mac':mac,
        'extension':extension,
        'ext_misc': ext_misc,
...
def application(environ,start_response):
    ...
    if isinstance(html_string, str):
        html_string = bytes(html_string, 'utf-8')
